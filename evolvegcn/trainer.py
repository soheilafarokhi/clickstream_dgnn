...
class Trainer():
    ...
    def train(self):
        ...
        for e in range(self.args.num_epochs):
            ...
            if len(self.splitter.dev) > 0 and e > self.args.eval_after_epochs:
                ...
                if eval_valid > best_eval_valid:
                    ...
                    self.logger.log_valid_measure('ep ' + str(e) + ' - Best valid measure:' + str(eval_valid))
                    # avg_cosine_sim = []
                    # valid_avg_cosine_sim = []
                    # for i in range(1, len(train_nodes_embs)):
                    #     prev_step = train_nodes_embs[i - 1].detach().cpu()
                    #     current_step = train_nodes_embs[i].detach().cpu()
                    #
                    #     prev_step_norm = prev_step / prev_step.norm(dim=1)[:, None]
                    #     current_step_norm = current_step / current_step.norm(dim=1)[:, None]
                    #     res = torch.mm(prev_step_norm, current_step_norm.transpose(0, 1))
                    #     avg = np.trace(res.numpy()) / res.shape[0]
                    #     avg_cosine_sim.append(avg)
                    #
                    #     prev_step = valid_node_embs[i - 1].detach().cpu()
                    #     current_step = valid_node_embs[i].detach().cpu()
                    #
                    #     prev_step_norm = prev_step / prev_step.norm(dim=1)[:, None]
                    #     current_step_norm = current_step / current_step.norm(dim=1)[:, None]
                    #     res = torch.mm(prev_step_norm, current_step_norm.transpose(0, 1))
                    #     avg = np.trace(res.numpy()) / res.shape[0]
                    #     valid_avg_cosine_sim.append(avg)
                    # plt.plot(avg_cosine_sim, label=f'epoch {e} train cosine sim')
                    # plt.plot(valid_avg_cosine_sim, label=f'epoch {e} valid cosine sim')
                    # plt.legend()
                    # plt.show()
        ...
        # plt.plot(testEpoch_loss, label='test_loss')
        # plt.legend()
        # plt.show()
    ...
    def predict(self, hist_adj_list, hist_ndFeats_list, node_indices, mask_list, raw_data_list=None):

        nodes_embs_list = self.gcn(hist_adj_list, hist_ndFeats_list,
                                       mask_list, raw_data_list)
        if ('lstm' in self.args.model) or ('gru' in self.args.model):
            nodes_embs = nodes_embs_list
        else:
    ...
    def prepare_sample(self, sample, set_name):
        ...
        for i, adj in enumerate(sample.hist_adj_list):
            ...
            sample.raw_data_list[i] = raw_data_list[i].to(self.args.device)
