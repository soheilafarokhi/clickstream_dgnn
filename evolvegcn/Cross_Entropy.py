...
class Cross_Entropy(torch.nn.Module):
    ...
    def __init__(self, args, dataset, num_classes):
        super().__init__()
    ...
    def forward(self, logits, labels):
        ...
        '''
        keep_indices = ((labels != (self.num_classes-1)).nonzero(as_tuple=True)[0])
        labels = labels.view(-1, 1)
        labels_keep = labels[keep_indices, :]
        logits = logits[keep_indices, :]
        alpha = self.weights(labels_keep)[labels_keep].view(-1, 1)
        loss = alpha * (- logits.gather(-1, labels_keep) + self.logsumexp(logits))
        return loss.mean()
