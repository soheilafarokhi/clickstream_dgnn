...
class Logger():
    def __init__(self, args, num_classes, minibatch_log_interval=10):
        ...
        if args is not None:
            ...
            if args.tuning:
                log_path = f"./log/tuning/{args.model}/{args.click_stream_args.course}_{str(num_classes - 1)}"
            else:
                log_path = f"./log/{args.model}/{args.click_stream_args.course}_{str(num_classes - 1)}"
            if not os.path.exists(log_path):
                os.makedirs(log_path)

            self.log_name= f'{log_path}/log_{str(args.num_hist_steps)}_weeks_{self.currdate}_r{str(args.rank)}.log'
            self.tuning_log_path = f'{log_path}/log_{str(args.num_hist_steps)}_weeks.txt'
            self.tuning_file = open(self.tuning_log_path, "a")
            self.tuning_file.write(f"\nModel timestamp: {self.currdate}\n")
            if args.use_logfile:
    ...
    def log_minibatch(self, predictions, true_classes, loss, **kwargs):
        ...
        if self.minibatch_done%self.minibatch_log_interval==0:
            ...
            for cl in range(self.num_classes - 1):
                ...
                weighted_precision += (cl_support * cl_precision)
                weighted_recall += (cl_support * cl_recall)
                weighted_f1 += (cl_support * cl_f1)
                support_sum += cl_support
                logging.info (self.set+' batch %d / %d - measures partial for class %d - precision %0.4f - recall %0.4f - f1 %0.4f - support %d' % (self.minibatch_done, self.num_minibatches, cl,cl_precision,cl_recall,cl_f1, cl_support))

            weighted_precision = weighted_precision/support_sum
            weighted_recall = weighted_recall/support_sum
            weighted_f1 = weighted_f1/support_sum
            logging.info(
                self.set + ' batch %d / %d - measures partial weighted avg - precision %0.4f - recall %0.4f - f1 %0.4f - support %d' % (
                self.minibatch_done, self.num_minibatches, weighted_precision, weighted_recall, weighted_f1, support_sum))
    ...
    def log_epoch_done(self, set_name):
        ...
        support_sum = 0
        for cl in range(self.num_classes -1):
            cl_support, cl_precision, cl_recall, cl_f1 = self.calc_eval_measures_per_class(self.conf_mat_tp, self.conf_mat_fn, self.conf_mat_fp, cl)
            weighted_precision += (cl_support * cl_precision)
            weighted_recall += (cl_support * cl_recall)
            weighted_f1 += (cl_support * cl_f1)
            support_sum += cl_support
        ...
        weighted_recall = weighted_recall / support_sum
        weighted_f1 = weighted_f1 / support_sum
        logging.info(self.set+' measures weighted avg - precision %0.4f - recall %0.4f - f1 %0.4f ' % (
                 weighted_precision, weighted_recall, weighted_f1))
        logging.info (self.set+' Total epoch time: '+ str(((time.monotonic()-self.ep_time))))

        if str(self.args.target_class) == 'W_AVG':
            if self.args.target_measure=='Precision' or self.args.target_measure=='prec':
                eval_measure = weighted_precision
            elif self.args.target_measure=='Recall' or self.args.target_measure=='rec':
                eval_measure = weighted_recall
            else:
    ...
    def log_valid_measure(self, text):
        self.tuning_file.write(f"\n{self.currdate} {text}\n")
